...
DROP TABLE IF EXISTS cdm.temp_src_person_location;
CREATE TABLE cdm.temp_src_person_location AS
SELECT
...
-- -------------------------------------------------------------------
-- Remove duplicates - procedure_occurrence
-- -------------------------------------------------------------------
...
CREATE UNIQUE INDEX ux_src_meas ON cdm.src_meas (participant_id, physical_measurements_id, measurement_id, code_value);
...
CREATE UNIQUE INDEX ux_src_meas_mapped ON cdm.src_meas_mapped ( participant_id, physical_measurements_id, measurement_id,
                                                                cv_concept_id, cv_source_concept_id, vu_concept_id,
                                                                vcv_concept_id, vcv_source_concept_id);
...
-- -------------------------------------------------------------------
-- Remove Remove duplicates - observation
-- -------------------------------------------------------------------
SELECT NULL INTO @partition_expr;
SELECT NULL INTO @last_part_expr;
SELECT NULL INTO @row_number;
...
DROP TABLE IF EXISTS cdm.tmp_measurement;

CREATE TABLE cdm.tmp_measurement AS
SELECT 
    @partition_expr := CONCAT_WS('-',
                        t.person_id,
                        ...
                        -- t.range_high,                    -- NULL
                        -- t.provider_id,                   -- NULL
                        COALESCE(t.visit_occurrence_id, 0),
                        t.measurement_source_value,
                        t.measurement_source_concept_id,
                        COALESCE(t.unit_source_value, 0),
                        COALESCE(t.value_source_value, 0)
        )                                                      AS partition_expr,
    @reset_num :=
        CASE
    ...
    t.person_id                                                 AS person_id,
    t.measurement_concept_id                                    AS measurement_concept_id,
    t.measurement_date                                          AS measurement_date,
    t.measurement_time                                          AS measurement_time,
    t.measurement_type_concept_id                               AS measurement_type_concept_id,
    t.operator_concept_id                                       AS operator_concept_id,
    t.value_as_number                                           AS value_as_number,
    t.value_as_concept_id                                       AS value_as_concept_id,
    t.unit_concept_id                                           AS unit_concept_id,
    ...
    t.provider_id                                               AS provider_id,
    t.visit_occurrence_id                                       AS visit_occurrence_id,
    t.measurement_source_value                                  AS measurement_source_value,
    t.measurement_source_concept_id                             AS measurement_source_concept_id,
    t.unit_source_value                                         AS unit_source_value,
    t.value_source_value                                        AS value_source_value,
    t.meas_id                                                   AS meas_id,
...
-- -------------------------------------------------------------------
-- Remove duplicates - condition_occurrence
-- -------------------------------------------------------------------
SELECT NULL INTO @partition_expr;
SELECT NULL INTO @last_part_expr;
SELECT NULL INTO @row_number;
...
DROP TABLE IF EXISTS cdm.tmp_condition_occurrence;

CREATE TABLE cdm.tmp_condition_occurrence AS
SELECT 
    @partition_expr := CONCAT_WS('-',
                        t.person_id,
                        t.condition_concept_id,
                        t.condition_start_date,
                        -- t.condition_end_date     -- NULL
    ...
    t.person_id                                                 AS person_id,
    t.condition_concept_id                                      AS condition_concept_id,
    t.condition_start_date                                      AS condition_start_date,
    t.condition_end_date                                        AS condition_end_date,
    t.condition_type_concept_id                                 AS condition_type_concept_id,
    t.stop_reason                                               AS stop_reason,
    t.provider_id                                               AS provider_id,
    t.visit_occurrence_id                                       AS visit_occurrence_id,
    t.condition_source_value                                    AS condition_source_value,
    t.condition_source_concept_id                               AS condition_source_concept_id,
    t.unit_id                                                   AS unit_id
FROM 
    cdm.condition_occurrence t
ORDER BY
    t.person_id,
    t.condition_concept_id,
    t.condition_start_date,
...
SELECT 
    ...
    t.unit_id
FROM
    cdm.tmp_condition_occurrence t
WHERE
    t.row_number = 1
;
