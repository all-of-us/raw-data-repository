...
class MetricsPipelineTest(testutil.CloudStorageTestBase):
  ...
  def test_end_to_end(self):
    ...
    for i in range(0, 12):
      all_metrics = metrics_list[i * 3]
      columbia_metrics = metrics_list[(i * 3) + 1]
      pitt_metrics = metrics_list[(i * 3) + 2]
      self.assertEquals(datetime.date(2016, 9, 1 + i), all_metrics.date)
      self.assertEquals('', all_metrics.hpoId)
      self.assertEquals(datetime.date(2016, 9, 1 + i), columbia_metrics.date)
      self.assertEquals('COLUMBIA', columbia_metrics.hpoId)
