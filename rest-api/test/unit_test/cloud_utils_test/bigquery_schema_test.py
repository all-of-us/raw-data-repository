...
from model.bq_base import BQField, BQRecordField, BQSchema, BQTable, BQFieldModeEnum, BQFieldTypeEnum, BQRecord
...
schemaFromBQ = [
  ...
  {"name": "nested", "type": "RECORD", "mode": "REPEATED",
   "description": "cloud_utils_test.bigquery_schema_test.BQTestNestedSchema", "fields": [
    {"name": "int_field", "type": "INTEGER", "mode": "NULLABLE"},
    {"name": "str_field", "type": "STRING", "mode": "NULLABLE"},
    {"name": "enum_field", "type": "INTEGER", "mode": "NULLABLE",
     "description": "cloud_utils_test.bigquery_schema_test.BQTestEnum"}
  ]}
]
...
class BigQueryRecordTest(unittest.TestCase):
  ...
  full_data = {
    ...
    'nested': [
      {
        'int_field': 10,
        'str_field': 'first string data',
        'enum_field': BQTestEnum.FIRST
      },
      {
        'int_field': 20,
        'str_field': 'second string data',
        'enum_field': BQTestEnum.SECOND
      },
      {
        'int_field': 30,
        'str_field': 'third string data',
        'enum_field': BQTestEnum.THIRD
      }
    ]
  }

  bq_data = {"descr": "str_field data",
             "timestamp": "2019-06-26T19:26:42.015372",
             "nested": [{"int_field": 10, "enum_field": 1, "str_field": "first string data"},
                        {"int_field": 20, "enum_field": 2,
                         "str_field": "second string data"},
                        {"int_field": 30, "enum_field": 3,
                         "str_field": "third string data"}]}
