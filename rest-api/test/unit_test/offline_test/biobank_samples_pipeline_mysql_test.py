...
_CSV_COLUMN_NAMES = ('biobank_id', 'sent_test', 'sent_count', 'sent_order_id',
                     'sent_collection_time', 'sent_finalized_time',
                     'source_site_name', 'source_site_consortium',
                     'source_site_mayolink_client_number', 'source_site_hpo',
                     'finalized_site_name', 'finalized_site_consortium',
                     'finalized_site_mayolink_client_number',
                     'finalized_site_hpo', 'finalized_username',
                     'received_test', 'received_count', 'received_sample_id',
                     'received_time', 'elapsed_hours',)
...
class MySqlReconciliationTest(FlaskTestBase):
  ...
  def _insert_order(self, participant, order_id, tests, order_time, finalized_time):
    order = BiobankOrder(
    ...
    id_1 = BiobankOrderIdentifier(
        system='https://orders.mayomedicallaboratories.com', value=order_id)
    id_2 = BiobankOrderIdentifier(
        system='https://www.pmi-ops.org', value='O%s' % order_id)
  ...
  def _submit_race_questionnaire_response(self, participant_id, race_answers):
    ...
    qr = make_questionnaire_response_json(
        participant_id, self._questionnaire_id, code_answers=code_answers)
  ...
  def test_reconciliation_query(self):
    ...
    file_time = order_time + datetime.timedelta(hours=23) + datetime.timedelta(
        minutes=59)
    ...
    self._insert_order(p_old_on_time, 'OldGoodOrder', BIOBANK_TESTS[:2],
                       old_order_time)
    self._insert_samples(p_old_on_time, BIOBANK_TESTS[:2],
                         ['OldGoodSample1',
                          'OldGoodSample2'], old_within_24_hours)
    ...
    o_late_and_missing = self._insert_order(p_late_and_missing, 'SlowOrder',
                                            BIOBANK_TESTS[:2], order_time)
    self._insert_samples(p_late_and_missing, [BIOBANK_TESTS[0]], ['LateSample'],
                         late_time)
    ...
    self._insert_samples(p_withdrawn_old_on_time, BIOBANK_TESTS[:2],
                         ['OldWithdrawnGoodSample1',
                          'OldWithdrawnGoodSample2'], old_within_24_hours)
    ...
    for repetition in xrange(3):
      self._insert_order(
          p_repeated,
          'RepeatedOrder%d' % repetition, [BIOBANK_TESTS[0]],
          two_days_ago + datetime.timedelta(hours=repetition))
      if repetition != 2:
        self._insert_samples(
            p_repeated, [BIOBANK_TESTS[0]], ['RepeatedSample%d' % repetition],
    ...
    self.assertItemsEqual(multi_sample_row['received_sample_id'].split(','),
