...
participants_<shard>.csv = ['date_of_birth', 'first_order_date', 'first_samples_arrived_date',
                            'first_physical_measurements_date', <questionnaire submission times>]
...
All results are mapped to (participant_id, date|metric) tuples, representing new
values for metrics on the dates in question. The reducer emits to GCS files
hpoId|metric|date|delta strings representing individual increments or decrements of the metric in
question for the given HPO on the given date.

The second MR reads the files generated by the first MR, maps them to (hpoId|metric, date|delta)
tuples, combines them in the combiner stage, and in the reducer stage emits
hpoId|metric|date|count to GCS files, representing counts for metrics for HPOs for each date
until today.
...
and in the reducer phase writes metric buckets to SQL.
...
class SummaryPipeline(pipeline.Pipeline):
  def run(self, bucket_name, now, input_files, version_id, parent_params=None):
    ...
    blob_key_2 = (yield mapreduce_pipeline.MapreducePipeline(
        ...
        combiner_spec='offline.metrics_pipeline.combine2',
    ...
    yield mapreduce_pipeline.MapreducePipeline(
        'Write Metrics',
        mapper_spec='offline.metrics_pipeline.map3',
        input_reader_spec='mapreduce.input_readers.GoogleCloudStorageInputReader',
        mapper_params=(yield BlobKeys(bucket_name, blob_key_2, now, version_id)),
        reducer_spec='offline.metrics_pipeline.reduce3',
        reducer_params={
            'version_id': version_id
        },
        shards=num_shards)
...
def map_participants(reader):
  ...
  (participantId, date|physicalMeasurements.COMPLETE)        (for physical measurements)
  ...
  for row in reader:
    ...
    for i in range(5, len(row)):
      ...
      if questionnaire_submitted_time:
        metric = QUESTIONNAIRE_MODULE_FIELD_NAMES[i - 5]
...
def combine2(key, new_values, old_values):  # pylint: disable=unused-argument
...
def reduce2(reducer_key, reducer_values, now=None):
...
def reduce3(reducer_key, reducer_values, version_id=None):
