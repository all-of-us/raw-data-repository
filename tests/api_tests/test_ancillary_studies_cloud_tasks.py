...
class AncillaryStudiesEnrollmentCloudTaskTest(BaseTestCase):
    ...
    @mock.patch("rdr_service.dao.rex_dao.RexParticipantMappingDao.get_from_ancillary_id")
    def test_update_participant_summary_for_nph_reconsent(self, mock_participant_mapping):
        aou_participant = self.data_generator.create_database_participant()
        self.data_generator.create_database_participant_summary(
            participant=aou_participant,
            nphDeactivation=True,
            nphDeactivationAuthored="2023-03-01T14:13:12",
        )
        mock_participant_mapping.return_value = ParticipantMapping(
            primary_participant_id=aou_participant.participantId
        )

        data = {
            "event_type": "consent",
            "participant_id": aou_participant.participantId,
            "event_authored_time": "2023-04-02T14:13:12",
        }
        from rdr_service.resource import main as resource_main
        ...
        self.assertTrue(ps.consentForNphModule1)
        self.assertEqual(ps.nphDeactivation, False)
        self.assertIsNone(ps.nphDeactivationAuthored)
