...
class EtmIngestionTest(BaseTestCase):
    def test_questionnaire_ingestion(self):
        ...
        questionnaire_obj: etm.EtmQuestionnaire = self.session.query(
            etm.EtmQuestionnaire
        ).one()
        self.assertEqual("emorecog", questionnaire_obj.questionnaire_type)
        self.assertEqual(
            questionnaire_json["identifier"][0]["value"],
            questionnaire_obj.semantic_version,
        )
    ...
    def test_questionnaire_versioning(self):
        ...
        questionnaire_list: List[etm.EtmQuestionnaire] = (
            self.session.query(etm.EtmQuestionnaire)
            .order_by(etm.EtmQuestionnaire.version)
            .all()
        )
    ...
    def test_questionnaire_response_ingestion(self):
        ...
        saved_response: etm.EtmQuestionnaireResponse = (
            self.session.query(etm.EtmQuestionnaireResponse)
            .filter(
                etm.EtmQuestionnaireResponse.etm_questionnaire_response_id
                == response["id"]
            )
            .one()
        )
        ...
        self.assert_has_extensions(
            expected_list=[
                EtmExtension(
                    type=etm.ExtensionType.METADATA,
                    key="response_device",
                    value="mouse",
                ),
                EtmExtension(
                ...
                EtmExtension(
                    type=etm.ExtensionType.METADATA, key="screen_height", value=1117
                ),
                EtmExtension(
                    type=etm.ExtensionType.METADATA, key="screen_width", value=1728
                ),
                ...
                EtmExtension(
                    type=etm.ExtensionType.METADATA, key="user_utc_offset", value=-300
                ),
                EtmExtension(
                    type=etm.ExtensionType.METADATA,
                    key="test_start_date_time",
                    value=1687521289096,
                ),
                EtmExtension(
                    type=etm.ExtensionType.METADATA,
                    key="test_end_date_time",
                    value=1687521390759,
                ),
                EtmExtension(
                    type=etm.ExtensionType.METADATA, key="test_duration", value=101663
                ),
                EtmExtension(
                    type=etm.ExtensionType.METADATA, key="test_restarted", value=0
                ),
                EtmExtension(
                    type=etm.ExtensionType.METADATA,
                    key="test_version",
                    value="EmoRecog_AoU.v1.May23",
                ),
                EtmExtension(
                    type=etm.ExtensionType.METADATA,
                    key="test_name",
                    value="TestMyBrain Multiracial Emotion Identification",
                ),
                EtmExtension(
                    type=etm.ExtensionType.METADATA,
                    key="test_short_name",
                ...
                EtmExtension(
                    type=etm.ExtensionType.METADATA, key="test_params", value="{}"
                ),
                EtmExtension(
                    type=etm.ExtensionType.METADATA, key="test_language", value="en"
                ),
                EtmExtension(type=etm.ExtensionType.OUTCOME, key="score", value=13),
                EtmExtension(
                    type=etm.ExtensionType.OUTCOME,
                    key="accuracy",
                    value=0.2708333333333333,
                ),
                EtmExtension(
                    type=etm.ExtensionType.OUTCOME, key="meanRTc", value=380.18
                ),
                EtmExtension(
                    type=etm.ExtensionType.OUTCOME, key="medianRTc", value=302.6
                ),
                ...
                EtmExtension(
                    type=etm.ExtensionType.OUTCOME, key="happy_meanRTc", value=264.9
                ),
                EtmExtension(
                    type=etm.ExtensionType.OUTCOME, key="happy_medianRTc", value=264.9
                ),
                EtmExtension(
                    type=etm.ExtensionType.OUTCOME, key="happy_sdRTc", value=None
                ),
                EtmExtension(
                    type=etm.ExtensionType.OUTCOME, key="angry_accuracy", value=0.25
                ),
                EtmExtension(
                    type=etm.ExtensionType.OUTCOME, key="angry_meanRTc", value=206.63
                ),
                EtmExtension(
                    type=etm.ExtensionType.OUTCOME, key="angry_medianRTc", value=253.5
                ),
                EtmExtension(
                    type=etm.ExtensionType.OUTCOME, key="angry_sdRTc", value=135.33
                ),
                EtmExtension(
                    type=etm.ExtensionType.OUTCOME,
                    key="fearful_accuracy",
                    value=0.5833333333333334,
                ),
                EtmExtension(
                    type=etm.ExtensionType.OUTCOME, key="fearful_meanRTc", value=496.19
                ),
                EtmExtension(
                    type=etm.ExtensionType.OUTCOME, key="fearful_medianRTc", value=306.2
                ),
                EtmExtension(
                    type=etm.ExtensionType.OUTCOME, key="fearful_sdRTc", value=388.6
                ),
                EtmExtension(
                    type=etm.ExtensionType.OUTCOME,
                    key="sad_accuracy",
                ...
                EtmExtension(
                    type=etm.ExtensionType.OUTCOME, key="sad_meanRTc", value=292.15
                ),
                EtmExtension(
                    type=etm.ExtensionType.OUTCOME, key="sad_medianRTc", value=292.15
                ),
                EtmExtension(
                    type=etm.ExtensionType.OUTCOME, key="sad_sdRTc", value=12.94
                ),
                EtmExtension(
                    type=etm.ExtensionType.OUTCOME, key="any_timeouts", value=0
                ),
                EtmExtension(
                    type=etm.ExtensionType.OUTCOME, key="flag_medianRTc", value=1
                ),
                EtmExtension(
                    type=etm.ExtensionType.OUTCOME, key="flag_sameResponse", value=0
                ),
                EtmExtension(
                    type=etm.ExtensionType.OUTCOME, key="flag_trialFlags", value=1
                ),
            ],
    ...
    def test_multiple_etm_tasks_for_retention(self):
        ...
        resp_authored = datetime.fromisoformat(
            questionnaire_response_json["authored"]
        ).replace(tzinfo=None)
        ...
        for etm_task_date in (
            resp_authored,
            resp_authored + timedelta(days=21),
            resp_authored + timedelta(days=10),
        ):
            questionnaire_response_json["authored"] = etm_task_date.strftime(
                "%Y-%m-%dT%H:%M:%S+00:00"
            )
            self.send_post(
                f"Participant/P{participant.participantId}/QuestionnaireResponse",
        ...
        ps_latest_etm_authored = datetime.fromisoformat(
            summary.get("latestEtMTaskAuthored")
        )
