...
INTERVALS = [INTERVAL_DAY, INTERVAL_WEEK, INTERVAL_MONTH, INTERVAL_QUARTER]
...
class CalendarDao(BaseDao):
    ...
    """
        if interval_key == INTERVAL_DAY:
            ...
            start_field = func.str_to_date(func.concat(func.yearweek(Calendar.day), "Sunday"), "%X%V%W")
            end_interval_offset = text("interval 1 week")
        elif interval_key == INTERVAL_MONTH:
            start_field = func.str_to_date(func.date_format(Calendar.day, "%Y%m01"), "%Y%m%d")
            end_interval_offset = text("interval 1 month")
        elif interval_key == INTERVAL_QUARTER:
            start_field = func.date(
                func.concat(
                    func.year(Calendar.day), "-", func.lpad((func.quarter(Calendar.day) - 1) * 3 + 1, 2, "0"), "-01"
                )
            )
        ...
        end_date_field = func.date_add(start_date_query.c.start_date, end_interval_offset).label("end_date")
