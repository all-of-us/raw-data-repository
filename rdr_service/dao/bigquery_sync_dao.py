...
class BQParticipantSummaryGenerator(object):
  ...
  """
    ...
    """
        ...
        organization = (
            session.query(Organization.externalId).filter(Organization.organizationId == p.organizationId).first()
        )
            ...
            "addresses": [
                {
                    ...
                    if qnan.StreetAddress_PIIState
                    else None,
    ...
    """
        code_id_query = (
            session.query(func.max(QuestionnaireConcept.codeId))
            .filter(QuestionnaireResponse.questionnaireId == QuestionnaireConcept.questionnaireId)
            .label("codeId")
        )
        query = (
            session.query(
    ...
    """
        ...
        query = (
            session.query(
                PhysicalMeasurements.created,
                PhysicalMeasurements.createdSiteId,
    ...
    """
        ...
        for consent in summary["consents"]:
            if consent["consent"] == "ConsentPII":
                study_consent = True
            if (
                consent["consent"] == "EHRConsentPII_ConsentPermission"
                and consent["consent_value"] == "ConsentPermission_Yes"
            ):
                ehr_consent = True
            if (
                consent["consent"] == "DVEHRSharing_AreYouInterested"
                and consent["consent_value"] == "DVEHRSharing_Yes"
            ):
                dvehr_consent = True
        ...
        if "modules" in summary:
            baseline_module_count = len(
                [module for module in summary["modules"] if module["mod_baseline_module"] == "true"]
            )
        if "biobank_orders" in summary:
            for order in summary["biobank_orders"]:
                if "samples" in order:
                    dna_sample_count += len(
                        [sample for sample in order["samples"] if sample["bbs_dna_test"] == "true"]
                    )
        ...
        if (
            pm_complete
            and "modules" in summary
            and baseline_module_count == len(baseline_modules)
            and dna_sample_count > 0
        ):
            ...
            for order in summary["biobank_orders"]:
                if order["bbo_status_id"] != int(BiobankOrderStatus.CANCELLED) and "bbo_samples" in order:
                    for sample in order["bbo_samples"]:
                        if (
                            "bbs_finalized" in sample
                            and sample["bbs_finalized"]
                            and isinstance(sample["bbs_finalized"], datetime.datetime)
                        ):
