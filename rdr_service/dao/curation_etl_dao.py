...
class CdrEtlRunHistoryDao(UpdatableDao):
    ...
    def create_etl_history_record(self, cutoff, vocabulary):
        now = clock.CLOCK.now()
        ...
        return self.insert(cdr_etl_run_history)

    def update_etl_end_time(self, etl_history_id):
        now = clock.CLOCK.now()
        with self.session() as session:
            record = self.get_for_update(session, etl_history_id)
            record.endTime = now

    def get_last_etl_run_info(self, is_sql=False):
        with self.session() as session:
            query = session.query(
                CdrEtlRunHistory.id,
...
class CdrExcludedCodeDao(BaseDao):
    ...
    def is_exist_exclude_code(self, code_value, code_type):
        with self.session() as session:
    ...
    def add_excluded_code(self, code_value, code_type):
        ...
        if not self.is_exist_exclude_code(code_value, code_type):
            cdr_excluded_code = CdrExcludedCode(
                codeId=code.codeId,
                codeValue=code_value,
                codeType=code_type
            )
            return self.insert(cdr_excluded_code)

    def remove_excluded_code(self, code_value, code_type):
        code_dao = CodeDao()
        code = code_dao.get_code(PPI_SYSTEM, code_value)
        if not code:
            raise ValueError(f'invalid code value: {str(code_value)}')
        if self.is_exist_exclude_code(code_value, code_type):
            with self.session() as session:
...
class CdrEtlSurveyHistoryDao(BaseDao):
    ...
    def get_last_etl_run_code_history(self, is_sql=False):
        run_history_dao = CdrEtlRunHistoryDao()
        etl_run_info = run_history_dao.get_last_etl_run_info()
        with self.session() as session:
            query = session.query(
    ...
    def save_include_exclude_code_history_for_etl_run(self, run_id):
        now = clock.CLOCK.now()
        with self.session() as session:
            # excluded codes
            column_map = {
