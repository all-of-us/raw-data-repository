...
class GoogleSheetCSVReader(csv.DictReader):
    ...
    def _get_sheet_url(sheet_id, gid):
        return ("https://docs.google.com/spreadsheets/d/{id}/export" "?format=csv" "&id={id}" "&gid={gid}").format(
            id=sheet_id, gid=gid
        )
    ...
    def _get_response_body(url):
        ...
        if response.status_code != 200:
            raise HttpException(" ".join(["Could not retreive response:", response.status_code, response.contents]))
