...
class DvOrderApi(UpdatableApi):
    ...
    def _put_supply_delivery(self, resource, bo_id):
        ...
        try:
            ...
            if hasattr(fhir["extension"], VIBRENT_FHIR_URL + "expected-delivery-date"):
                eta = dateutil.parser.parse(
                    fhir.extension.get(url=VIBRENT_FHIR_URL + "expected-delivery-date").valueDateTime
                )

            tracking_status = fhir.extension.get(url=VIBRENT_FHIR_URL + "tracking-status").valueString
            tracking_id = fhir.identifier.get(system=VIBRENT_FHIR_URL + "trackingId").value
        except AttributeError as e:
            raise BadRequest(e.message)
        except Exception as e:
            raise BadRequest(e.message)
        ...
        tracking_status_enum = getattr(
            OrderShipmentTrackingStatus, tracking_status.upper(), OrderShipmentTrackingStatus.UNSET
        )

        biobank_dv_order_id = self.dao.get_id(ObjDict({"participantId": p_id, "order_id": int(bo_id)}))
