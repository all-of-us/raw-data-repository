...
def import_codebook():
  ...
  """
    ...
    if new_codebook_version is None:
        response["error_messages"] = ['Published codebook is missing "version", import aborted.']
    ...
    if new_codebook_version == response["active_version"]:
        response["status_messages"] = ["Version %s already active, not importing." % response["active_version"]]
        return _log_and_return_json(response)
    ...
    if GAE_PROJECT == 'localhost':
        ...
        task = taskqueue.add(
            ...
            url=TASK_PREFIX + 'BQRebuildCodebookTaskApi',
            method='GET',
            target='worker',
            params=None
        )
