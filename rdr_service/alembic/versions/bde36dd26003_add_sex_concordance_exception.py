"""add sex_concordance_exception

Revision ID: bde36dd26003
Revises: 401d853d41d0
Create Date: 2025-01-31 10:18:08.507738
...
revision = 'bde36dd26003'
down_revision = '401d853d41d0'
branch_labels = None
depends_on = None
...
def upgrade_rdr():
    ...
    op.add_column('genomic_aw3_raw', sa.Column('sex_concordance_exception', sa.String(length=255), nullable=True))
    op.create_index(op.f('ix_genomic_aw3_raw_sex_concordance_exception'), 'genomic_aw3_raw', ['sex_concordance_exception'], unique=False)
    op.add_column('genomic_set_member', sa.Column('sex_concordance_exception', sa.String(length=255), nullable=True))
    op.add_column('genomic_set_member_history', sa.Column('sex_concordance_exception', sa.String(length=255), nullable=True))
    op.execute("""
...
NEW.cvl_secondary_conf_failure <=> OLD.cvl_secondary_conf_failure OR
NEW.diversion_pouch_site_flag <=> OLD.diversion_pouch_site_flag OR
NEW.informing_loop_ready_flag <=> OLD.informing_loop_ready_flag OR
NEW.informing_loop_ready_flag_modified <=> OLD.informing_loop_ready_flag_modified OR
NEW.sex_concordance_exception <=> OLD.sex_concordance_exception
                    );
...
def downgrade_rdr():
    ...
    op.drop_column('genomic_set_member', 'sex_concordance_exception')


    op.drop_index(op.f('ix_genomic_aw3_raw_sex_concordance_exception'), table_name='genomic_aw3_raw')
    op.drop_column('genomic_aw3_raw', 'sex_concordance_exception')
    op.drop_column('genomic_set_member_history', 'sex_concordance_exception')
    op.execute("""
            DROP TRIGGER IF EXISTS genomic_set_member__ai;
            DROP TRIGGER IF EXISTS genomic_set_member__au;
            DROP TRIGGER IF EXISTS genomic_set_member__bd;
