...
"""
from rdr_service import config, participant_enums
from rdr_service.census_regions import census_regions
from rdr_service.code_constants import BASE_VALUES, PPI_SYSTEM, UNSET
from rdr_service.dao.code_dao import CodeDao
from rdr_service.dao.hpo_dao import HPODao
from rdr_service.dao.participant_summary_dao import ParticipantSummaryDao
from rdr_service.field_mappings import (
    CONSENT_FOR_ELECTRONIC_HEALTH_RECORDS_FIELD,
    CONSENT_FOR_STUDY_ENROLLMENT_FIELD,
...
def _get_answer_values(question_code_value):
    ...
    if not q_code:
        return []
    return [answer_code.value for answer_code in q_code.children if answer_code.codeType == CodeType.ANSWER]
...
def get_participant_fields():
    return BASE_PARTICIPANT_FIELDS + [
...
CONFIG = {
    ...
    + [FieldDef(fieldname, _get_submission_statuses) for fieldname in QUESTIONNAIRE_MODULE_FIELD_NAMES]
    + [
        CodeIdFieldDef(fieldname, _get_answer_values_func(question_code))
        for fieldname, question_code in ANSWER_FIELD_TO_QUESTION_CODE.items()
    ],
    # These fields are computed using the first provided function in the definition,
    # based on the state of other fields.
    "summary_fields": [
        SummaryFieldDef(BIOSPECIMEN_SUMMARY_METRIC, _biospecimen_summary, _get_biospecimen_summary_values),
        SummaryFieldDef(
            CONSENT_FOR_STUDY_ENROLLMENT_AND_EHR_METRIC,
            _consent_for_study_enrollment_and_ehr,
            _get_submission_statuses,
        ),
        SummaryFieldDef(
            NUM_COMPLETED_BASELINE_PPI_MODULES_METRIC,
            _num_completed_baseline_ppi_modules,
            _get_completed_baseline_ppi_modules_values,
        ),
        SummaryFieldDef(ENROLLMENT_STATUS_METRIC, _enrollment_status, _get_enrollment_statuses),
    ],
}

PARTICIPANT_SUMMARY_FIELD_TO_METRIC_FIELD = {
    f.participant_summary_field: f.name for f in CONFIG["fields"] if f.participant_summary_field
}
...
def get_fields():
    ...
    for field in conf["fields"] + conf["summary_fields"]:
        field_dict = {"name": PARTICIPANT_KIND + "." + field.name, "values": field.values_func()}
