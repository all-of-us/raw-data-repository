...
from rdr_service.participant_enums import BiobankOrderStatus, OrganizationType, \
  get_sample_status_enum_value
...
def write_reconciliation_report(now, report_type='daily'):
  ...
  bucket_name = config.getSetting(config.BIOBANK_SAMPLES_BUCKET_NAME)  # raises if missing
  _query_and_write_reports(SqlExporter(bucket_name, use_unicode=True), now, report_type,
