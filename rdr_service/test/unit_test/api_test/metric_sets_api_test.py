...
class MetricSetsApiTest(FlaskTestBase):
    ...
    @parameterized.expand(
        [
            ...
            param(
                "all",
                want={
                    "GENDER": [{"value": "female", "count": 123}, {"value": "male", "count": 789}],
                    "STATE": [{"value": "NJ", "count": 789}, {"value": "CA", "count": 123}],
                },
            ),
            param(
                "proper key subset",
                keys=["STATE"],
                want={"STATE": [{"value": "NJ", "count": 789}, {"value": "CA", "count": 123}]},
            ),
            param(
                "overlapping key subset",
                keys=["STATE", "AGE_RANGE"],
                want={"STATE": [{"value": "NJ", "count": 789}, {"value": "CA", "count": 123}]},
            ),
    ...
    def test_get_metrics(self, _, ms_id="live", keys=None, want=None):
        ...
        self.aggregate_metrics_dao.insert(
            AggregateMetrics(metricSetId="live", metricsKey=MetricsKey.GENDER, value="female", count=123)
        )
        self.aggregate_metrics_dao.insert(
            AggregateMetrics(metricSetId="live", metricsKey=MetricsKey.GENDER, value="male", count=789)
        )
        self.aggregate_metrics_dao.insert(
            AggregateMetrics(metricSetId="live", metricsKey=MetricsKey.STATE, value="NJ", count=789)
        )
        self.aggregate_metrics_dao.insert(
            AggregateMetrics(metricSetId="live", metricsKey=MetricsKey.STATE, value="CA", count=123)
        )
        q = {"keys": keys} if keys else None
        got = self.send_get("MetricSets/{}/Metrics".format(ms_id), query_string=q)["metrics"]
        self.assertEqual(
            len(want), len(got), "got unexpected number of metrics:" "\nwant:  {}\ngot: {}".format(want, got)
        )
        for m in got:
    ...
    def test_get_metrics_bad_keys(self):
        self.create_metric_set("live")
        self.send_get("MetricSets/live/Metrics", query_string={"keys": "mugman"}, expected_status=400)
