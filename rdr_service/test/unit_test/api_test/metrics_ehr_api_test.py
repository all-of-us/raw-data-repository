...
class MetricsEhrApiTestBase(FlaskTestBase):
    def setUp(self, **kwargs):
        ...
        self.hpo_test = self._make_hpo(
            hpoId=TEST_HPO_ID, name=TEST_HPO_NAME, displayName="Test", organizationType=OrganizationType.UNSET
        )
    ...
    def _update_ehr(self, participant_summary, update_time):
        receipt = EhrReceipt(organizationId=participant_summary.organizationId, receiptTime=update_time)
...
class MetricsEhrMultiEndpointTest(MetricsEhrApiTestBase):
  ...
  """
    ...
    def setUp(self, **kwargs):
        ...
        #
        for date in iter_dates(datetime.date(2017, 12, 30), datetime.date(2018, 2, 1)):
    ...
    def test_combined_endpoint_matches_parts(self):
        ...
        participants_over_time_response = self.send_get("MetricsEHR/ParticipantsOverTime")
        organizations_response = self.send_get("MetricsEHR/Organizations")
...
class MetricsEhrApiTest(MetricsEhrApiTestBase):
    def test_consented_counts(self):
        ...
        response = self.send_get("MetricsEHR", query_string=urllib.parse.urlencode({"organization": "foo_a"}))
...
class MetricsEhrApiOrganizationTest(MetricsEhrApiTestBase):
    def test_cutoff_date_filtering(self):
        ...
        response = self.send_get(
            ...
            query_string=urllib.parse.urlencode({"end_date": "2018-01-01", "interval": "day"}),
        )
        self.assertEqual(
        ...
        response = self.send_get(
            "MetricsEHR/Organizations",
            query_string=urllib.parse.urlencode({"end_date": "2018-01-02", "interval": "day"}),
        )
        self.assertEqual(
            response[str(self.org_foo_a.externalId)],
            {
        ...
        response = self.send_get(
            "MetricsEHR/Organizations",
            query_string=urllib.parse.urlencode({"end_date": "2018-01-03", "interval": "day"}),
        )
        self.assertEqual(
            response[str(self.org_foo_a.externalId)],
            {
        ...
        response = self.send_get(
            "MetricsEHR/Organizations",
            query_string=urllib.parse.urlencode({"end_date": "2018-01-04", "interval": "day"}),
        )
        self.assertEqual(
            response[str(self.org_foo_a.externalId)],
            {
        ...
        response = self.send_get(
            "MetricsEHR/Organizations",
            query_string=urllib.parse.urlencode({"end_date": "2018-01-05", "interval": "day"}),
        )
        self.assertEqual(
            response[str(self.org_foo_a.externalId)],
            {
        ...
        response = self.send_get(
            "MetricsEHR/Organizations",
            query_string=urllib.parse.urlencode({"end_date": "2018-01-06", "interval": "day"}),
        )
        self.assertEqual(
            response[str(self.org_foo_a.externalId)],
            {
