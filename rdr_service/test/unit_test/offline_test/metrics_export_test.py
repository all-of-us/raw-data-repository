...
from rdr_service.participant_enums import WithdrawalReason, WithdrawalStatus, make_primary_provider_link_for_name
  ...
  """
    ...
    def _create_data(self):
        ...
        with FakeClock(TIME):
            ...
            participant9 = Participant(participantId=9, biobankId=10, providerLink=pl_pitt)
            participant_dao.insert(participant9)
            self.send_consent("P9", email="mike@gmail.com")
        ...
        with FakeClock(TIME_3):
            ...
            self.submit_questionnaire_response(
                "P2", questionnaire_id_2, race_code=None, gender_code=None, state="PIIState_VA", date_of_birth=None
            )
            self.submit_questionnaire_response(
                "P6", questionnaire_id_2, race_code=None, gender_code=None, state="PIIState_VA", date_of_birth=None
            )
            ...
            self.submit_consent_questionnaire_response("P6", questionnaire_id_3, CONSENT_PERMISSION_YES_CODE)

            path = "Participant/P9/BiobankOrder"
            new_order = self.send_post(path, load_biobank_order_json(9))
            path = path + "/" + new_order["id"]
            self.send_get(path)
            self.send_patch(path, request_data=cancel_biobank_order(), headers={"If-Match": 'W/"1"'})
    ...
    def assertBucket(self, bucket_map, dt, hpoId, metrics=None):
        ...
        if metrics:
            ...
            self.assertMultiLineEqual(pretty(metrics), pretty(json.loads(bucket.metrics)))
        else:
