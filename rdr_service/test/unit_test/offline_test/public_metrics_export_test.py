...
class PublicMetricsExportTest(CloudStorageSqlTestBase, FlaskTestBase):
    ...
    def _create_data(self):
        ...
        SqlTestBase.setup_codes(
            list(ANSWER_FIELD_TO_QUESTION_CODE.values()) + [EHR_CONSENT_QUESTION_CODE], code_type=CodeType.QUESTION
        )
        SqlTestBase.setup_codes(list(FIELD_TO_QUESTIONNAIRE_MODULE_CODE.values()), code_type=CodeType.MODULE)
        ...
        with FakeClock(TIME):
            ...
            participant3 = Participant(
                participantId=3, biobankId=4, providerLink=make_primary_provider_link_for_name("TEST")
            )
            ...
            participant4 = Participant(
                participantId=4, biobankId=5, providerLink=make_primary_provider_link_for_name("PITT")
            )
            participant_dao.insert(participant4)
            self.send_consent("P4", email="bob@example.com")

            participant5 = Participant(
                participantId=5, biobankId=6, providerLink=make_primary_provider_link_for_name("PITT")
            )
            ...
            participant6 = Participant(
                participantId=6, biobankId=7, providerLink=make_primary_provider_link_for_name("PITT")
            )
            ...
            self.submit_questionnaire_response(
                "P1", questionnaire_id, RACE_WHITE_CODE, "female", None, datetime.date(1980, 1, 2)
            )
            self.submit_questionnaire_response(
                "P2", questionnaire_id, PMI_PREFER_NOT_TO_ANSWER_CODE, "male", None, datetime.date(1920, 1, 3)
            )
            self.submit_questionnaire_response("P2", questionnaire_id_2, None, None, "PIIState_VA", None)
            self.submit_questionnaire_response("P5", questionnaire_id, None, None, None, datetime.date(1970, 1, 2))
            self.submit_consent_questionnaire_response("P1", questionnaire_id_3, CONSENT_PERMISSION_NO_CODE)
            self.submit_consent_questionnaire_response("P2", questionnaire_id_3, CONSENT_PERMISSION_YES_CODE)
            sample_dao = BiobankStoredSampleDao()
            sample_dao.insert(
                BiobankStoredSample(
                    biobankStoredSampleId="abc", biobankId=2, biobankOrderIdentifier="KIT", test="test", confirmed=TIME
                )
            )
            sample_dao.insert(
                BiobankStoredSample(
                    biobankStoredSampleId="def", biobankId=3, biobankOrderIdentifier="KIT", test="1SAL", confirmed=TIME
                )
            )
            # Required to update the HPO linkage (and test filtering for P3).
            sample_dao.insert(
                BiobankStoredSample(
                    biobankStoredSampleId="xyz", biobankId=4, biobankOrderIdentifier="KIT", test="1SAL", confirmed=TIME
                )
            )
    ...
    def assert_total_count_per_key(self, want_total_count):
        ...
        for (k, aggs) in agg_by_key.items():
            ...
            self.assertEqual(
                ...
                count,
                (
                    "metric {} must contain aggregates over exactly " "the set of {} qualified participants, got {}"
                ).format(k, want_total_count, count),
            )
