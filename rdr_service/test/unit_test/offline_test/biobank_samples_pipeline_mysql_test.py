...
from rdr_service.code_constants import BIOBANK_TESTS, PPI_SYSTEM, RACE_AIAN_CODE, \
  RACE_QUESTION_CODE, RACE_WHITE_CODE
...
from rdr_service.test.unit_test.unit_test_util import FlaskTestBase, InMemorySqlExporter, \
...
class MySqlReconciliationTest(FlaskTestBase):
  ...
  def test_reconciliation_query(self):
    ...
    biobank_samples_pipeline._query_and_write_reports(exporter, file_time, 'daily',
  ...
  def test_monthly_reconciliation_report(self):
    ...
    received, missing, modified, withdrawals = 'rx_monthly.csv', 'missing_monthly.csv', \
                                               'modified_monthly.csv', 'withdrawals_monthly.csv'
    exporter = InMemorySqlExporter(self)
    biobank_samples_pipeline._query_and_write_reports(exporter, file_time, 'monthly',
                                                      received, missing, modified, withdrawals)
