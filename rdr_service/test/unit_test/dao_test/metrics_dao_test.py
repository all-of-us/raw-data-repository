...
from rdr_service.dao.metrics_dao import MetricsBucketDao, MetricsVersionDao, SERVING_METRICS_DATA_VERSION
...
class MetricsDaoTest(SqlTestBase):
    ...
    def test_set_pipeline_in_progress(self):
        ...
        expected_mv = MetricsVersion(
            metricsVersionId=1, inProgress=True, complete=False, date=TIME, dataVersion=SERVING_METRICS_DATA_VERSION
        )
        self.assertEqual(expected_mv.asdict(), self.metrics_version_dao.get(1).asdict())
        self.assertEqual(expected_mv.asdict(), self.metrics_version_dao.get_version_in_progress().asdict())
        self.assertIsNone(self.metrics_version_dao.get_serving_version())
        self.assertIsNone(self.metrics_bucket_dao.get_active_buckets())
    ...
    def test_set_pipeline_in_progress_while_in_progress(self):
        ...
        with FakeClock(TIME_3):
            self.metrics_version_dao.set_pipeline_in_progress()
        expected_mv = MetricsVersion(
            metricsVersionId=1, inProgress=False, complete=False, date=TIME, dataVersion=SERVING_METRICS_DATA_VERSION
        )
        self.assertEqual(expected_mv.asdict(), self.metrics_version_dao.get(1).asdict())
        expected_mv2 = MetricsVersion(
            metricsVersionId=2, inProgress=True, complete=False, date=TIME_3, dataVersion=SERVING_METRICS_DATA_VERSION
        )
    ...
    def test_set_pipeline_finished_in_progress_no_buckets(self):
        ...
        with FakeClock(TIME_2):
            self.metrics_version_dao.set_pipeline_finished(True)
        expected_mv = MetricsVersion(
            metricsVersionId=1, inProgress=False, complete=True, date=TIME, dataVersion=SERVING_METRICS_DATA_VERSION
        )
        self.assertEqual(expected_mv.asdict(), self.metrics_version_dao.get(1).asdict())
    ...
    def test_set_pipeline_finished_in_progress_with_buckets(self):
        with FakeClock(TIME):
            self.metrics_version_dao.set_pipeline_in_progress()
        metrics_bucket_1 = MetricsBucket(metricsVersionId=1, date=datetime.date.today(), hpoId="", metrics="foo")
        metrics_bucket_2 = MetricsBucket(metricsVersionId=1, date=datetime.date.today(), hpoId=PITT, metrics="bar")
        tomorrow = datetime.date.today() + datetime.timedelta(days=1)
        metrics_bucket_3 = MetricsBucket(metricsVersionId=1, date=tomorrow, hpoId="", metrics="baz")
        ...
        self.assertEqual(expected_mv.asdict(), self.metrics_version_dao.get_serving_version().asdict())
    ...
    def test_insert_duplicate_bucket(self):
        with FakeClock(TIME):
            self.metrics_version_dao.set_pipeline_in_progress()
        metrics_bucket_1 = MetricsBucket(metricsVersionId=1, date=datetime.date.today(), hpoId=PITT, metrics="foo")
        metrics_bucket_2 = MetricsBucket(metricsVersionId=1, date=datetime.date.today(), hpoId=PITT, metrics="bar")
        self.metrics_bucket_dao.insert(metrics_bucket_1)
        ...
        self.assertEqual(
            metrics_bucket_2.asdict(), self.metrics_bucket_dao.get([1, datetime.date.today(), PITT]).asdict()
        )
