...
EXAMPLE_SUPPLY_REQUEST = {
    ...
    "contained": [
        ...
        },
        {
            ...
            "id": "patient-1",
            "identifier": [{"system": "participantId", "value": "P748018940"}],
    ...
    "extension": [
        ...
        {"url": "http://vibrenthealth.com/fhir/fulfillment-status", "valueString": "pending"},
    ],
    "identifier": [{"code": "99993", "system": "orderId"}],
...
class SimpleFhirR4ReaderBasicTestCase(unittest.TestCase):
    ...
    def test_list_lookup(self):
        fhir = SimpleFhirR4Reader([{"a": "foo", "b": 0}, {"a": "bar", "b": 1}])
    ...
    def test_looks_up_top_level_references(self):
        fhir = SimpleFhirR4Reader({"contained": [{"id": "some-foo", "value": 123}], "foo": {"reference": "#some-foo"}})
        self.assertEqual(fhir.foo.value, 123)

    def test_looks_up_deep_references(self):
        fhir = SimpleFhirR4Reader(
            {"contained": [{"id": "some-foo", "value": 123}], "bar": {"foo": {"reference": "#some-foo"}}}
        )
...
class SimpleFhirR4ReaderSupplyRequestTestCase(unittest.TestCase):
    ...
    def test_lookup_patient_id(self):
        self.assertEqual(
            self.fhir.contained.get(resourceType="Patient").identifier.get(system="participantId").value, "P748018940"
        )
