...
class ParticipantTest(BaseClientTest):
    def test_create_and_modify_participant(self):
        ...
        provider_link = {"primary": True, "organization": {"reference": "Organization/AZ_TUCSON"}}

        # Add a provider link to the participant.
        provider_link_2 = {"primary": True, "organization": {"reference": "Organization/PITT"}}
        # Create a new participant.
        participant = {"providerLink": [provider_link_2]}
        ...
        new_response = self.client.request_json(
            "Participant/%s" % participant_id, "PUT", response, headers={"If-Match": 'W/"1"'}
        )
        ...
        try:
            response = self.client.request_json("Participant/{}".format(participant_id), "PUT", response)
            self.fail("Need If-Match header for update")
        except HttpException as ex:
            self.assertEqual(ex.code, 400)
        try:
            response = self.client.request_json(
                "Participant/{}".format(participant_id), "PUT", response, headers={"If-Match": 'W/"12345"'}
            )
            self.fail("Wrong If-Match header for update")
        except HttpException as ex:
            self.assertEqual(ex.code, 412)
        new_response = self.client.request_json(
            "Participant/{}".format(participant_id), "PUT", new_response, headers={"If-Match": last_etag}
        )
        ...
        # pair with awardee
        self.client.request_json(
            "Participant/{}".format(participant_id), "PUT", response, headers={"If-Match": last_etag}
        )
        ...
        self.client.request_json(
            "Participant/{}".format(participant_id), "PUT", updated_response, headers={"If-Match": last_etag}
        )
