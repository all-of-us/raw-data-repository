...
FROM
  `{dataset_id}.tmp_fact_rel_sd` tmp1
INNER JOIN
  `{dataset_id}.tmp_fact_rel_sd` tmp2
ON
  tmp1.person_id = tmp2.person_id
...
WHERE
  tmp1.systolic_blood_pressure_ind != 0              -- take only systolic blood pressure measurements
...
FROM
  `{dataset_id}.measurement` cdm_meas
WHERE
  cdm_meas.parent_id IS NOT NULL
UNION ALL
SELECT
  21 AS domain_concept_id_1,
  -- Measurement
...
FROM
  `{dataset_id}.observation` cdm_obs
INNER JOIN
  `{dataset_id}.measurement_to_qualifier` mtq
ON
  mtq.qualifier_id = cdm_obs.meas_id
  UNION ALL
  SELECT
  21 AS domain_concept_id_1,
  -- Measurement
...
ON
  mtq.qualifier_id = cdm_obs.meas_id
       """, 'destination': 'fact_relationship', 'append': False},
...
HAVING
 ...
 """, 'destination': 'src_race', 'append': False},
    'src_race_2': {'query': """
  SELECT
  DISTINCT src_m.participant_id AS person_id,
  MIN(stcm1.source_code) AS ppi_code,
...
HAVING
  COUNT(DISTINCT src_m.value_ppi_code) = 1
       """, 'destination': 'src_race', 'append': True},
    'src_ethnicity': {'query': """
        SELECT
  DISTINCT src_m.participant_id AS person_id,
  MIN(stcm1.source_code) AS ppi_code,
...
HAVING
  COUNT(DISTINCT src_m.value_ppi_code) = 1
       """, 'destination': 'src_ethnicity', 'append': False},
    'src_ethnicity_2': {'destination': 'src_ethnicity', 'append': True,
                        'query': """
                          SELECT
  DISTINCT src_m.participant_id AS person_id,
  MIN(stcm1.source_code) AS ppi_code,
...
FROM
  `{dataset_id}.site` site
       """, 'destination': 'care_site', 'append': False},
    'location': {'query': """
SELECT
...
FROM (
   ...
   `{dataset_id}.src_person_location` src
       ...
       """, 'destination': 'location', 'append': False},
    'measurement': {'query': """
    SELECT
...
WHERE
  ...
  OR meas.cv_domain_id IS NULL
   """, 'destination': 'measurement', 'append': False},
    'note': {'query': """
        SELECT
  NULL AS note_id,
  meas.participant_id AS person_id,
...
ON
  src_m.participant_id = person_loc.participant_id
       """, 'destination': 'person', 'append': False},
    'src_gender': {'query': """
      SELECT
  DISTINCT src_m.participant_id AS person_id,
  MIN(stcm1.source_code) AS ppi_code,
...
HAVING
  COUNT(DISTINCT src_m.value_ppi_code) = 1
     """, 'destination': 'src_gender', 'append': False},
    'src_mapped': {'query': """
       SELECT
...
WHERE src_c.filter = 0
       """, 'destination': 'src_mapped', 'append': False},
    'src_meas_mapped': {'query': """
        SELECT
...
WHERE
  meas.code_value <> 'notes'""", 'destination': 'src_meas_mapped', 'append': False},
    'src_participant': {'query': """
        SELECT
  f1.participant_id,
...
FROM (
  ...
  GROUP BY
    ...
    t1.src_id ) f1
       """, 'destination': 'src_participant', 'append': False},
    'src_person_location': {'query': """
        SELECT
...
GROUP BY
  `{dataset_id}.src_participant`.participant_id;
       """, 'destination': 'src_person_location', 'append': False},
    'tmp_cv_concept_lk': {'query': """
       SELECT
...
WHERE
  meas.code_value IS NOT NULL
      """, 'destination': 'tmp_cv_concept_lk', 'append': False},
    'tmp_fact_rel_sd': {'query': """
      SELECT
...
WHERE
  ...
  AND m.parent_id IS NOT NULL
     """, 'destination': 'tmp_fact_rel_sd', 'append': False},
    'tmp_vcv_concept_lk': {'query': """
        SELECT
...
WHERE
  meas.value_code_value IS NOT NULL
       """, 'destination': 'tmp_vcv_concept_lk', 'append': False},
    'tmp_visits_src': {'query': """
        SELECT
...
GROUP BY
  ...
  src_meas.src_id
       """, 'destination': 'tmp_visits_src', 'append': False},
    'visit_occurrence': {'query': """
        SELECT
...
FROM
  `{dataset_id}.tmp_visits_src` src
       """, 'destination': 'visit_occurrence', 'append': False},
    'observation': {'query': """
        SELECT
...
WHERE
  meas.cv_domain_id = 'Observation'
       """, 'destination': 'observation', 'append': False},
    'src_clean_qrids': {'query': """
        SELECT
...
WHERE
  FILTER = 0 ;
       """, 'destination': 'src_clean_qrids', 'append': False},
    'filter_questions': {'query': """
    UPDATE
  `{dataset_id}.src_clean`
SET
...
WHERE
  ...
  FROM
    `{dataset_id}.combined_question_filter`)""", 'destination': None, 'append': False},
    'filter_surveys': {'query': """
    UPDATE
  `{dataset_id}.src_clean`
SET
  `{dataset_id}.src_clean`.filter = 1
WHERE
  ...
  FROM
   ...
   """, 'destination': None, 'append': False},
    'update_location_id': {'append': False, 'query': """
    UPDATE
  `{dataset_id}.src_person_location` person_loc
SET
...
WHERE
  ...
  AND IFNULL(person_loc.zip, '') = IFNULL(loc.zip, '')
    """, 'destination': None},
    'survey_conduct': {'destination': 'survey_conduct', 'append': False,
                       'query': """SELECT
  *
FROM
  EXTERNAL_QUERY("all-of-us-rdr-prod.us-central1.bq-rdr-preprod-curation",
    "SELECT qr.questionnaire_response_id survey_conduct_id, p.participant_id person_id,    COALESCE(voc_c.concept_id, 0) survey_concept_id,    CAST(NULL AS DATE) survey_start_date,    CAST(NULL AS DATETIME) survey_start_datetime,    DATE(qr.authored) survey_end_date,    qr.authored survey_end_datetime,    0 provider_id,    CASE      WHEN qr.non_participant_author = 'CATI' THEN 42530794    ELSE    0  END    assisted_concept_id,    0 respondent_type_concept_id,    0 timing_concept_id,    CASE      WHEN qr.non_participant_author = 'CATI' THEN 42530794    ELSE    42531021  END    collection_method_concept_id,    CASE      WHEN qr.non_participant_author = 'CATI' THEN 'Telephone'    ELSE    'No matching concept'  END    assisted_source_value,    CAST(NULL AS CHAR) respondent_type_source_value,    '' timing_source_value,    CASE      WHEN qr.non_participant_author = 'CATI' THEN 'Telephone'    ELSE    'Electronic'  END    collection_method_source_value,    mc.value survey_source_value,    mc.code_id survey_source_concept_id,    qr.questionnaire_response_id survey_source_identifier,    0 validated_survey_concept_id,    '' validated_survey_source_value,    CAST(NULL AS UNSIGNED) survey_version_number,    '' visit_occurrence_id,    '' response_visit_occurrence_id,    p.participant_origin src_id  FROM    questionnaire_response qr  INNER JOIN    questionnaire_concept qc  ON    qc.questionnaire_id = qr.questionnaire_id    AND qc.questionnaire_version = qr.questionnaire_version  INNER JOIN    code mc  ON    mc.code_id = qc.code_id  INNER JOIN    participant p  ON    p.participant_id = qr.participant_id  LEFT JOIN    voc.concept voc_c  ON    voc_c.concept_code = mc.value    AND voc_c.vocabulary_id = 'PPI'    AND voc_c.domain_id = 'observation'    AND voc_c.concept_class_id = 'module'  WHERE    qr.questionnaire_response_id IN (    SELECT      DISTINCT sc.questionnaire_response_id    FROM      cdm.src_clean sc    WHERE      sc.filter = 0 )");
                       """},
    'death': {'destination': 'death', 'append': False,
              'query': """SELECT
...
WHERE
  dr.status = 2"""},
    'ehr_consent_temp_table': {'destination': 'tmp_ehr_consent', 'append': False,
                               'query': """SELECT
  *
FROM
  EXTERNAL_QUERY("all-of-us-rdr-prod.us-central1.bq-rdr-preprod-curation",
    "SELECT sc.participant_id, sc.research_id, sc.value_ppi_code, sc.date_of_survey, sc.src_id, consent_file.created"""\
    """ AS cf_created, consent_file.sync_status, consent_response.created AS cr_created FROM cdm.src_clean sc """\
    """LEFT JOIN rdr.consent_response ON sc.questionnaire_response_id = consent_response.questionnaire_response_id """\
    """LEFT JOIN rdr.consent_file ON consent_response.id = consent_file.consent_response_id WHERE sc.survey_name = """\
    """'EHRConsentPII' AND sc.question_ppi_code = 'EHRConsentPII_ConsentPermission'");"""},
    'ehr_consent': {'destination': 'consent', 'append': False,
                    'query': """SELECT ec.participant_id AS person_id,
ec.research_id,
CASE
WHEN ec.value_ppi_code IN ('No','ConsentPermission_No') THEN 'SUBMITTED_NO'
WHEN ec.value_ppi_code IN ('Yes','ConsentPermission_Yes') AND cr_created IS NULL THEN 'SUBMITTED'
WHEN ec.value_ppi_code IN ('Yes','ConsentPermission_Yes') AND ec.cr_created IS NOT NULL AND ec.cf_created IS NULL THEN 'SUBMITTED_NOT_VALIDATED'
WHEN ec.value_ppi_code IN ('Yes','ConsentPermission_Yes') AND ec.cf_created > '{cutoff}' THEN 'SUBMITTED_NOT_VALIDATED'
WHEN ec.value_ppi_code IN ('Yes','ConsentPermission_Yes') AND ec.sync_status IN (2,4) THEN 'SUBMITTED'
WHEN ec.value_ppi_code IN ('Yes','ConsentPermission_Yes') AND ec.sync_status NOT IN (2,4) THEN 'SUBMITTED_INVALID'
...
FROM `{dataset_id}.tmp_ehr_consent` ec"""},
    'wear_consent': {'destination': 'wear_consent', 'append': False,
    ...
    'participant_id_mapping': {'destination': 'participant_id_mapping', 'append': False,
                               'query':
