...
def check_ppi_data(client, args):
  ...
  """
    ...
    response_json = client.request_json("CheckPpiData", method="POST", body={"ppi_data": ppi_data})
...
if __name__ == "__main__":
    ...
    parser = get_parser()
    parser.add_argument(
        "spreadsheet_id", help='Google spreadsheet doc ID, after the "/d/" in the URL. The doc must be public.'
    )
    parser.add_argument("spreadsheet_gid", help='Google spreadsheet sheet ID, after "gid=" in the URL.')
    parser.add_argument(
        "--email",
        help=(
            "Only validate the given e-mail(s). Validate all by default."
