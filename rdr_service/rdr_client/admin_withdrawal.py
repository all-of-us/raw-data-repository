...
def main(client):
    ...
    for participant in participants:
        ...
        if not client.args.dry_run:
            response = client.request_json(path, "PUT", request_body, headers={"If-Match": client.last_etag})
            logging.info(
                "\n Participant: {} withdrawn. New info: \n {}".format(
                    response["participantId"], pprint.pformat(response)
                )
            )
        else:
            logging.info(
                "Request that would be sent for participant {}: \n {} ".format(
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument(
        "--withdrawal_reason", help="withdrawal reason is one of [fraudulent | " "duplicate | test]", required=True
    )
    arg_parser.add_argument(
        "--withdrawal_justification", help="A string justification for " "withdrawal", required=True, nargs="+"
    )
    arg_parser.add_argument(
        "--participants",
        help="Participants to withdrawal, all having " "the same reason and justification. Seperated by " "spaces",
        required=True,
        nargs="+",
    )
