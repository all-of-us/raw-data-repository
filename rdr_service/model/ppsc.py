...
class AccountLinkageEvent(PPSCBase):
    __tablename__ = "account_linkage_event"
...
event.listen(AccountLinkageEvent, "before_insert", model_insert_listener)
event.listen(AccountLinkageEvent, "before_update", model_update_listener)
...
class Site(PPSCBase):
    ...
    mayo_link_id = Column(String(128), nullable=False)
    enrollment_status_active = Column(TINYINT, default=0, nullable=False)
    active = Column(TINYINT, default=0, nullable=False)
