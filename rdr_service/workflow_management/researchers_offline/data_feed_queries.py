...
def insert_workbench_workspaces_staging_data(project: str, destination_dataset: str, last_job_run_date: str) -> str:
    ...
    # Get all active workspace data
        ...
        SELECT DISTINCT
            ...
            -- CASE
              ...
              -- ELSE ""
            -- END AS access_tier
            "" AS access_tier, -- TODO: Temporary value while waiting on access to this table
        ...
        WHERE ww.workspace_display_name IS NOT NULL
        UNION ALL
    ...
    # All other fields are set to null. Null data will be filled in through the RDR MySQL table
        SELECT DISTINCT
            ...
            FALSE AS excludeFromPublicDirectory,
...
def export_workbench_workspaces_staging_data_multi_region(project: str, source_dataset: str) -> str:
    return f"""
        EXPORT DATA OPTIONS(
            uri='gs://all-of-us-rdr-stable-cdm-new/data/workbench_stable_test_data_*.json',
            format='JSON',
            compression='NONE',
            overwrite=TRUE
        ) AS
        ...
        FROM (
            SELECT *
            FROM `{project}.{source_dataset}.datafeed_input_workbench_workspaces`
        )
    """


def insert_workbench_workspaces_staging_data_multi_region(project: str, destination_dataset: str) -> str:
    return f"""
        CREATE OR REPLACE EXTERNAL TABLE `{project}.{destination_dataset}.workbench_data_transfer_external`
        (
            workspace_source_id STRING NOT NULL,
            name STRING NOT NULL,
            creation_time TIMESTAMP,
            modified_time TIMESTAMP,
            status STRING,
            exclude_from_public_directory BOOLEAN,
            disease_focused_research STRING,
            disease_focused_research_name STRING,
            other_purpose_details STRING,
            methods_development STRING,
            control_set STRING,
            ancestry STRING,
            social_behavioral STRING,
            population_health STRING,
            drug_development STRING,
            commercial_purpose STRING,
            educational STRING,
            other_purpose STRING,
            resource STRING,
            findings_from_study STRING,
            intend_to_study STRING,
            access_to_care STRING,
            age ARRAY<STRING>,
            disability_status STRING,
            education_level STRING,
            ethical_legal_social_implications STRING,
            focus_on_underrepresented_populations STRING,
            gender_identity STRING,
            geography STRING,
            income_level STRING,
            others STRING,
            race_ethnicity ARRAY<STRING>,
            scientific_approaches STRING,
            sex_at_birth STRING,
            sexual_orientation STRING,
            review_requested STRING,
            is_reviewed STRING,
            workspace_users ARRAY<STRUCT<role STRING, user_email STRING, status STRING>>,
            creator STRUCT<user_id INT64, given_name STRING, family_name STRING>,
            cdr_version STRING,
            access_tier STRING,
            aian_research_details STRING,
            aian_research_type STRING
        )
        OPTIONS (
            format = 'JSON',
            uris =['gs://all-of-us-rdr-stable-cdm-new/data/workbench_stable_test_data_*.json']
        )
    """


def create_workspace_source_id_mapping(project: str, dataset: str, mapping_table: str, data_transfer_table: str) -> str:
    """
    """
    return f"""
        ...
        FROM `{project}.{dataset}.{data_transfer_table}` wdte
        WHERE NOT EXISTS (
            SELECT 1
            FROM `{project}.{dataset}.{mapping_table}` wsim
...
def get_workbench_workspaces_data_to_stream(project: str, destination_dataset: str, mapping_table: str, data_transfer_table: str) -> str:
    ...
    return f"""
        SELECT
            dtt.* EXCEPT (workspace_source_id, creation_time, modified_time),
            mt.legacy_workspace_source_id AS workspace_source_id,
            DATETIME(dtt.creation_time, 'UTC') AS creation_time,
            DATETIME(dtt.modified_time, 'UTC') AS modified_time
        FROM `{project}.{destination_dataset}.{data_transfer_table}` dtt
        JOIN `{project}.{destination_dataset}.{mapping_table}` mt ON mt.workspace_source_id = dtt.workspace_source_id
        WHERE NOT EXISTS (
            SELECT 1
            ...
            AND rwws.modified_time = DATETIME(dtt.modified_time, 'UTC')
        )
    """
