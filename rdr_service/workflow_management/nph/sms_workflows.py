...
def get_nph_slack_message_handler() -> SlackMessageHandler:
...
class SmsWorkflow:
    ...
    def job_ingestion(self):
        ...
        if not str(self.file_path).endswith(".csv"):
            ...
            nph_slack = get_nph_slack_message_handler()
            nph_slack.send_message_to_webhook(
                message_data=f"Error ingesting nph file of type {file_end}. "
                             "File does not conform to csv format."
            )
            return
