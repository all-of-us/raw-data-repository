...
class QuestionnaireGen(BaseGen):
  ...
  """
    ...
    """
        ...
        q_resp.questionnaire = FHIRReference(
            {
                "reference": "Questionnaire/{0}/_history/{1}".format(
                    self._module_questions.id, self._module_questions.version
                )
            }
        )
    ...
    """
        ...
        data["phone"] = "{0:03}-{1:03}-{2:04}".format(
            random.randint(100, 999), random.randint(0, 999), random.randint(0, 9999)
        )
    ...
    """
        ...
        qnans.append(self._answer_question(self._find_question("PIIName_First"), contact["first"]))
        qnans.append(self._answer_question(self._find_question("PIIName_Middle"), contact["middle"]))
        qnans.append(self._answer_question(self._find_question("PIIName_Last"), contact["last"]))
        qnans.append(self._answer_question(self._find_question("ConsentPII_EmailAddress"), contact["email"]))
        qnans.append(self._answer_question(self._find_question("PIIContactInformation_Email"), contact["email"]))
        qnans.append(self._answer_question(self._find_question("PIIContactInformation_Phone"), contact["phone"]))
        qnans.append(
            self._answer_question(self._find_question("ConsentPII_VerifiedPrimaryPhoneNumber"), contact["phone"])
        )
        qnans.append(self._answer_question(self._find_question("PIIAddress_StreetAddress"), contact["street1"]))
        qnans.append(self._answer_question(self._find_question("PIIAddress_StreetAddress2"), contact["street1"]))
        qnans.append(self._answer_question(self._find_question("StreetAddress_PIICity"), contact["city"]))
        qnans.append(self._answer_question(self._find_question("StreetAddress_PIIState"), contact["state"]))
        qnans.append(self._answer_question(self._find_question("StateOfResidence"), contact["state"]))
        qnans.append(self._answer_question(self._find_question("ReceiveCare_PIIState"), contact["state"]))
        qnans.append(self._answer_question(self._find_question("StreetAddress_PIIZIP"), contact["zip"]))
        qnans.append(
            self._answer_question(self._find_question("PIIBirthInformation_BirthDate"), contact["dob"].isoformat())
        )
        qnans.append(self._answer_question(self._find_question("ExtraConsent_WelcomeVideo"), clock.CLOCK.now()))
        qnans.append(self._answer_question(self._find_question("ExtraConsent_AgreeToConsent"), clock.CLOCK.now()))

        # q = self._find_question('Language')
        # q = self._find_question('Language_SpokenWrittenLanguage')

        optional = [
            "ExtraConsent_21YearsofAge",
            "ExtraConsent_19YearsofAge",
            "ExtraConsent_HealthRecordVideo",
            "ExtraConsent_CABoRSignature",
            "ExtraConsent__CABoRTodaysDate",
            "VeteransHealthAdministration",
            "ExtraConsent_KeepinginTouchVideo",
            "ExtraConsent_HealthDataVideo",
            "ExtraConsent_PhysicalMeausrementsVideo",
            "ExtraConsent_SamplesVideo",
            "ExtraConsent_DNAVideo",
            "ExtraConsent_FitnessTrackerVideo",
            "ExtraConsent_OtherHealthDataVideo",
            "ExtraConsent_DataSharingVideo",
            "ExtraConsent_RisktoPrivacyVideo",
            "ExtraConsent_EmailACopyToMe",
            "ExtraConsent_AllOfUsPurpose",
            "ExtraConsent_TakePartAllOfUs",
            "ExtraConsent_CanIWithdraw",
            "ExtraConsent_PrivacyGuaranteeQuestion",
            "ExtraConsent_ReadyOrNeedHelp",
            "ExtraConsent_AgreeToConsent",
            "ExtraConsent_Signature",
            "ExtraConsent_TodaysDate",
            "ConsentPII_HelpWithConsent",
    ...
    """
        ...
        if random.random() > 0.6:
            contact = self._make_contact_info(self._find_question("PersonOneAddress_PersonOneAddressState"))

            qnans.append(
                self._answer_question(self._find_question("SecondaryContactInfo_PersonOneFirstName"), contact["first"])
            )
            qnans.append(
                self._answer_question(
                    self._find_question("SecondaryContactInfo_SecondContactsMiddleInitial"), contact["middle"][:1]
                )
            )
            qnans.append(
                self._answer_question(self._find_question("SecondaryContactInfo_PersonOneLastName"), contact["last"])
            )
            qnans.append(
                self._answer_question(self._find_question("SecondaryContactInfo_PersonOneTelephone"), contact["phone"])
            )
            qnans.append(
                self._answer_question(self._find_question("SecondaryContactInfo_PersonOneEmail"), contact["email"])
            )
            ...
            qnans.append(
                self._answer_question(self._find_question("PersonOneAddress_PersonOneAddressCity"), contact["city"])
            )
            qnans.append(
                self._answer_question(self._find_question("PersonOneAddress_PersonOneAddressState"), contact["state"])
            )
            qnans.append(
                self._answer_question(self._find_question("PersonOneAddress_PersonOneAddressZipCode"), contact["zip"])
            )
        else:
            if random.random() > 0.5:
                qnans.append(
                    self._answer_question(self._find_question("SecondaryContactInfo_PersonOnePreferNotToAnswer"), True)
                )
        ...
        for code in edu_codes:
            edu_choices.append(
                (code, 3.5)
                if code
                in ["HighestGrade_TwelveOrGED", "HighestGrade_CollegeOnetoThree", "HighestGrade_CollegeGraduate"]
                else (code, 0.5)
            )
        ...
        optional = [
            ...
            "ActiveDuty_AvtiveDutyServeStatus",
        ...
        if question.type == "boolean":
            ...
            if item:
                if int(item["decimal_answer_count"]) > 0:
                    answer = round(
                        random.uniform(float(item["min_decimal_answer"]), float(item["max_decimal_answer"])), 1
                    )
            else:
            ...
            if item:
                if int(item["datetime_answer_count"]) > 0:
                    ...
                    answer = (min_date + datetime.timedelta(seconds=random.randint(0, seconds_diff))).isoformat()
            else:
                days_sub = random.randint(1, (5 * 365))
                answer = (clock.CLOCK.now() - datetime.timedelta(days=days_sub)).isoformat()
            ...
            if item:
                if int(item["string_answer_count"]) > 0:
                    answer = " ".join(
                        [random.choice(self._app_data["latin_words"]) for _ in range(random.randint(1, 5))]
                    )
            else:
                answer = " ".join([random.choice(self._app_data["latin_words"]) for _ in range(random.randint(1, 5))])
