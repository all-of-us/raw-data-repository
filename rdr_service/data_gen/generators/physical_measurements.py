...
class PhysicalMeasurementsGen(BaseGen):
  ...
  def _make_physical_measurements(self, force_measurement=False):
    ...
    entries = [{
      ...
      "resource":
        {"author": [self._make_author("creator@pmi-ops.org", 'created'),
         ...
         "extension": [{"url": created_ext,
                        "valueReference": "{0}{1}".format('Location/', self._site.id)
                        },
                       {"url": finalized_ext,
                        "valueReference": "{0}{1}".format('Location/', self._site.id)
                        }],
         ...
         "status": "final",
         "subject": {
           "reference": "Patient/%s" % self._participant_id
         },
    ...
    for measurement in self._measurement_specs:
      ...
      first_entry = self._make_measurement_entry(measurement, now, self._participant_id,
      ...
      if num_measurements > 1:
        ...
        entries.append(self._make_mean_entry(measurement, now,
                                             self._participant_id, measurement_resources))
    ...
    for qualifier in qualifier_set:
      ...
      entry = self._make_measurement_entry(qualifier_measurement, now, self._participant_id,
                                           qualifier_set)
      entries.append(entry)
