...
class SampleAvailabilityDatasetTool(ToolBase):
    def run(self):
        ...
        # print('count check')
        # for participant_id in aliquots_by_participant:
        #     for sample_type in aliquots_by_participant[participant_id]:
        #         aliquots = aliquots_by_participant[participant_id][sample_type]
        #         total_count = len(aliquots)
        #         collection_dates = set()
        #         passable_count = 0
        #         for a in aliquots:
        #             if a.meets_quantity_reqs():
        #                 passable_count += 1
        #                 collection_dates.add(a.collection_timestamp)
        #         if passable_count > 1 and len(collection_dates) > 1:
        #             print(f'{participant_id} -- passable {sample_type}: {passable_count} (total: {total_count})', collection_dates)
        ...
        with open('export.csv', 'w') as file:
            ...
            for data in data_to_export:
                writer.writerow({
                    ...
                    'pst_plasma_collection_timestamp': data.pst_plasma_availability.collection_datetime if data.pst_plasma_availability.is_available else None,
                    'edta_plasma_availability': 'Y' if data.edta_plasma_availability.is_available else 'N',
                    'edta_plasma_collection_timestamp': data.edta_plasma_availability.collection_datetime if data.edta_plasma_availability.is_available else None,
                    'serum_availability': 'Y' if data.serum_availability.is_available else 'N',
                    'serum_collection_timestamp': data.serum_availability.collection_datetime if data.serum_availability.is_available else None,
                    'blood_availability': 'Y' if data.blood_availability.is_available else 'N',
                    'blood_collection_timestamp': data.blood_availability.collection_datetime if data.blood_availability.is_available else None,
                    'saliva_availability': 'Y' if data.saliva_availability.is_available else 'N',
                    'saliva_collection_timestamp': data.saliva_availability.collection_datetime if data.saliva_availability.is_available else None
                })
    ...
    def retrieve_eligible_participant_ids(cls, session: Session) -> List[int]:
        query = sa.select(
            ...
            AwardeeInSite.firstEhrReceiptTime.is_not(None)
        )
        ...
        result = session.execute(query.distinct()).all()
    ...
    def filter_query_by_survey(cls, survey_name, query: sa.orm.query) -> sa.orm.query:
        participant_event = sa.orm.aliased(ppsc.ParticipantEventActivity)
...
def add_additional_arguments(parser: argparse.ArgumentParser):
    ...
    # )
    # parser.add_argument(
    #     '--upload-file', help='Upload an audited report to the bucket.', default=None
    # )


def run():
    return cli_run(tool_cmd, tool_desc, SampleAvailabilityDatasetTool, add_additional_arguments)
