...
class DigestBackfillTool(ToolBase):
    ...
    def update_id_list(self):
        ...
        if num_ids > 2500:
            logger.error(
                f"Max of 2500 questionnaire_response_ids can be backfilled. File contained {num_ids} ids"
            )
        elif num_ids:
            with self.get_session() as session:
                questionnaire_response_query = (
                ...
                for response in questionnaire_response_query:
                    answer_hash = QuestionnaireResponseDao.calculate_answer_hash(
                        json.loads(response.resource)
                    )
