  ...
  """
    ...
    updated."""
        ...
        with open(filename, "r") as csv_file:
            ...
            existing_map = {getattr(entity, self.external_id_field): entity for entity in self.dao.get_all()}
        ...
        if self.errors:
            for err in self.errors:
                logging.warn(err)
        logging.info(
            "Done importing %ss%s: %d skipped, %d new, %d updated, %d not changed, " "%d deleted, %d errors.",
            self.entity_name,
    ...
    def _update_entity(self, entity, existing_entity, session, dry_run):
        ...
        if existing_dict == new_dict:
            ...
            logging.info(
                log_prefix + 'Updating %s "%s": changes = %s',
                self.entity_name,
                new_dict[self.external_id_field],
                changes,
            )
            if not dry_run:
