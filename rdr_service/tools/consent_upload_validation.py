...
def get_bucket_file_info(participant_ids, bucket, p_dict=None):
    ...
    for _id in participant_ids:
        ...
        if bucket == base_bucket:
            ...
            gsutil_command = shlex.split(
                str("gsutil ls gs://" + bucket + "/Participant/" + "hpo-site-" + p_dict[_id] + "/" + _id)
            )
        try:
...
def main(args):
    ...
    participant_files_awardee_bucket = get_bucket_file_info(participant_ids, args.bucket, participant_dict)
...
if __name__ == "__main__":
    configure_logging()
    parser = get_parser()
    parser.add_argument("--bucket", help="The source bucket to check against. e.g. aouXXX", required=True)
    parser.add_argument(
        "--input",
        help="Path to input csv. The csv should at a minimum contain"
