...
class HPOImporter(CsvImporter):
    def __init__(self):
        super(HPOImporter, self).__init__(
            "awardee", HPODao(), "hpoId", "name", [HPO_AWARDEE_ID_COLUMN, HPO_NAME_COLUMN, HPO_TYPE_COLUMN]
        )
        self.new_count = 0
    ...
    def _entity_from_row(self, row):
        ...
        try:
            ...
            logging.warn("Invalid organization type %s for awardee %s", type_str, row[HPO_AWARDEE_ID_COLUMN])
            self.errors.append(
                "Invalid organization type {} for awardee {}".format(type_str, row[HPO_AWARDEE_ID_COLUMN])
            )
            return None
        return HPO(
            name=row[HPO_AWARDEE_ID_COLUMN].upper(),
        ...
        if hpos_to_remove:
            ...
            for hpo in hpos_to_remove:
                ...
                if old_hpo and old_hpo.isObsolete != ObsoleteStatus.OBSOLETE:
                    ...
                    logging.info("Not attempting to delete hpo [%s] with existing obsolete status", old_hpo.name)

            if hpo_id_list and not dry_run:
                logging.info(log_prefix + "Marking old HPO as obsolete: %s", old_hpo.name)
                str_list = ",".join([str(i) for i in hpo_id_list])

                sql = """ UPDATE HPO
            SET is_obsolete = 1
            WHERE hpo_id in ({params})""".format(
                    params=str_list
                )
...
class OrganizationImporter(CsvImporter):
    def __init__(self):
        super(OrganizationImporter, self).__init__(
            ...
            "organizationId",
            "externalId",
            [ORGANIZATION_AWARDEE_ID_COLUMN, ORGANIZATION_ORGANIZATION_ID_COLUMN, ORGANIZATION_NAME_COLUMN],
        )
        self.hpo_dao = HPODao()
        self.environment = None
    ...
    def _entity_from_row(self, row):
        ...
        if hpo is None:
            logging.warn(
                "Invalid awardee ID %s importing organization %s",
            ...
            self.errors.append(
                "Invalid awardee ID {} importing organization {}".format(
                    row[ORGANIZATION_AWARDEE_ID_COLUMN], row[ORGANIZATION_ORGANIZATION_ID_COLUMN]
                )
            )
            return None
        return Organization(
    ...
    def _cleanup_old_entities(self, session, row_list, dry_run):
        ...
        org_group_list_from_sheet = [row[ORGANIZATION_ORGANIZATION_ID_COLUMN].upper() for row in row_list]
        ...
        if orgs_to_remove:
            ...
            for org in orgs_to_remove:
                ...
                if old_org and old_org.isObsolete != ObsoleteStatus.OBSOLETE:
                    ...
                    logging.info(
                        "Not attempting to delete org [%s] with existing obsolete status", old_org.displayName
                    )
...
class SiteImporter(CsvImporter):
    def __init__(self):
        ...
        super(SiteImporter, self).__init__(
            "site",
            SiteDao(),
            "siteId",
            "googleGroup",
            [
                SITE_ORGANIZATION_ID_COLUMN,
                SITE_SITE_ID_COLUMN,
                SITE_SITE_COLUMN,
                SITE_STATUS_COLUMN + self.environment,
        ...
        if not dry_run:
            if self.environment:
                ...
                if self.environment.strip() == "STABLE" and len(self.new_sites_list) > 0:
                    ...
                    request = service.apps().services().versions().list(appsId=current_env, servicesId="default")
                    ...
                    for version in versions["versions"]:
                        if version["servingStatus"] == "SERVING":
                            ...
                            request = (
                                ...
                                .versions()
                                .instances()
                                .list(servicesId="default", versionsId=_id, appsId=current_env)
                            )
                            ...
                            try:
                                for instance in instances["instances"]:
                                    ...
                                    delete_instance = (
                                        ...
                                        .instances()
                                        .delete(
                                            appsId=current_env, servicesId="default", versionsId=_id, instancesId=sha
                                        )
                                    )
    ...
    def _cleanup_old_entities(self, session, row_list, dry_run):
        ...
        for site in sites_to_remove:
            ...
            if old_site and old_site.isObsolete != ObsoleteStatus.OBSOLETE:
                ...
                logging.info("Not attempting to delete site [%s] with existing obsolete status", old_site.googleGroup)
    ...
    def _entity_from_row(self, row):
        ...
        organization = self.organization_dao.get_by_external_id(row[SITE_ORGANIZATION_ID_COLUMN].upper())
        if organization is None:
            logging.warn(
                "Invalid organization ID %s importing site %s", row[SITE_ORGANIZATION_ID_COLUMN].upper(), google_group
            )
            self.errors.append(
        ...
        if launch_date_str:
            try:
                ...
                self.errors.append("Invalid launch date {} for site {}".format(launch_date_str, google_group))
                return None
        ...
        if mayolink_client_number_str:
            try:
                ...
                logging.warn("Invalid Mayolink Client # %s for site %s", mayolink_client_number_str, google_group)
                self.errors.append(
                    "Invalid Mayolink Client # {} for site {}".format(mayolink_client_number_str, google_group)
                )
                return None
        ...
        try:
            ...
            logging.warn(
                "Invalid site status %s for site %s", row[SITE_STATUS_COLUMN + self.environment], google_group
            )
            self.errors.append(
                "Invalid site status {} for site {}".format(row[SITE_STATUS_COLUMN + self.environment], google_group)
            )
            return None
        try:
            ...
            logging.warn(
                "Invalid enrollment site status %s for site %s",
    ...
    def _populate_lat_lng_and_time_zone(self, site, existing_site):
        if site.address1 and site.city and site.state:
            if existing_site:
                if (
                    existing_site.address1 == site.address1
            ...
            if site.googleGroup not in self.status_exception_list:
                if site.siteStatus == self.ACTIVE:
                    self.errors.append(
                        "Active site must have valid address. Site: {}, Group: {}".format(
    ...
    def _get_lat_long_for_site(self, address_1, city, state):
        ...
        try:
            ...
            self.errors.append("Geocoding failured Check that address is correct. ERROR: {}".format(self.api_key, e))
            return None, None
...
if __name__ == "__main__":
    configure_logging()
    parser = get_parser()
    parser.add_argument("--awardee_file", help="Filename containing awardee CSV to import", required=True)
    parser.add_argument("--organization_file", help="Filename containing organization CSV to import", required=True)
    parser.add_argument("--site_file", help="Filename containing site CSV to import", required=True)
    parser.add_argument("--dry_run", help="Read CSV and check for diffs against database.", action="store_true")
    parser.add_argument(
        "--stub_geocoding",
    ...
    parser.add_argument("--creds_file", type=str, help="Path to credentials JSON file.")
