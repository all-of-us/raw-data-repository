...
def _create_questionnaire_response(participant_id, q_id_and_version, questions_with_answers):
    qr_json = {
        "resourceType": "QuestionnaireResponse",
        "status": "completed",
        "subject": {"reference": "Patient/%s" % participant_id},
        "questionnaire": {"reference": "Questionnaire/%s/_history/%s" % (q_id_and_version[0], q_id_and_version[1])},
        "group": {},
    }
...
def _submit_questionnaire_response(client, participant_id, questionnaire_id_and_version, questions, answer_map):
    ...
    if not questions_with_answers:
        ...
        return
    qr_json = _create_questionnaire_response(participant_id, questionnaire_id_and_version, questions_with_answers)
...
def import_participant(
    row, client, consent_questionnaire_id_and_version, questionnaire_to_questions, consent_questions, reader=None
):
    ...
    if hpo_id:
        participant_resource["providerLink"] = [
            {"primary": True, "organization": {"display": None, "reference": "Organization/%s" % hpo_id}}
        ]
    ...
    if not reader:
        client.request_json("Participant/%s" % participant_id, "PUT", row, headers={"If-Match": 'W/"1"'})

    _submit_questionnaire_response(
        client, participant_id, consent_questionnaire_id_and_version, consent_questions, answer_map
    )
    for questionnaire_id_and_version, questions in questionnaire_to_questions.items():
        if questionnaire_id_and_version != consent_questionnaire_id_and_version:
            _submit_questionnaire_response(client, participant_id, questionnaire_id_and_version, questions, answer_map)
    if reader:
        logging.info("%s created: (%r %r).", participant_id, row["first_name"], row["last_name"])
    else:
        logging.info("%s created from (%r %r).", participant_id, row["first_name"], row["last_name"])


if __name__ == "__main__":
    configure_logging()
    parser = get_parser()
    parser.add_argument("--file", help="Path to the CSV file containing the participant data.", required=True)
    parser.add_argument(
        "--instance",
        type=str,
